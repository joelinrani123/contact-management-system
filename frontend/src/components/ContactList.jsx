...
const ContactList = forwardRef(({ contacts, setContacts }, ref) => {
  ...
  // Expose fetchContacts to parent via ref
  ...
  return (
    <>
      ...
      {loading && (
        <div className="w-100 d-flex flex-column align-items-center justify-content-center rounded p-3 mt-4 gap-2" style={{ height: "300px" }}>
          <img src="/spinning-dots.svg" alt="loading" width={40} height={40} />
          <p className="mt-1 fw-bold" style={{ color: "navy", fontSize: "16px" }}>Loading...</p>
        </div>
      )}

      {!loading && contacts.length === 0 && (
        <div className="w-100 d-flex flex-column align-items-center justify-content-center rounded p-3 mt-4 gap-2 bg-light" style={{ height: "300px" }}>
          <img src="/ChatGPT Image Dec 3, 2025, 10_34_07 AM.png" alt="no contacts" width={200} height={200} />
          <p className="mt-1 fw-bold" style={{ color: "navy", fontSize: "16px" }}>No contacts found.</p>
        </div>
      )}
          ...
          <div key={c._id}>
            <div className="bg-light shadow rounded p-3 d-flex flex-column justify-content-between">
              <div className="d-flex gap-2 mb-2 justify-content-between align-items-center">
                ...
                <p className="px-2 py-1 rounded" style={{ backgroundColor: "#d3e6ff", fontWeight: 500, fontSize: "12px" }}>{c.company}</p>
              </div>
              ...
              <div className="d-flex justify-content-between align-items-center mt-2">
                ...
                <button className="btn btn-danger px-2 py-1" style={{ fontSize: "13px" }} onClick={() => handleDelete(c._id)}>Delete</button>
              </div>
            </div>
          </div>
        ))}
